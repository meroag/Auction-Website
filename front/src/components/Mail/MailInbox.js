...
export default function MailInbox() {
  ...
  useEffect(()=>{
      ...
      axios.get(`https://stagingzix.xyz/mail/inbox/${authState.id}`, head).then((res)=>{
  ...
  const displayMessage = (name) =>{
    ...
    if (name.read===false){
      ...
      axios.put(`https://stagingzix.xyz/mail/message/read/${name.id}`).then((res)=>{
      });
  ...
  const deleteMessage = (name) =>{
    ...
    axios.put(`https://stagingzix.xyz/mail/message/deleteinbox/${name.id}`).then((res)=>{
    });
    let mails = [...mailList];
