...
export default function MailMessage() {
  ...
  const messageSend = () =>{
    ...
    if ( Object.keys(personContact).length === 0 ){
      ...
      axios.post(`https://stagingzix.xyz/mail/message/${authState.id}`, body, head).then((res)=>{
          if (res.data.error){
  ...
  const handleChange = (event) => {
    ...
    axios.get(`https://stagingzix.xyz/mail/auctions/${authState.id}`, head).then((res)=>{
...
useEffect(()=>{
  ...
  axios.get(`https://stagingzix.xyz/mail/contacts/${authState.id}`, head).then((res)=>{
    console.log(res.data);
    setMails(res.data);
