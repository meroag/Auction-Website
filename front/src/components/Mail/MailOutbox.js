...
export default function MailOutbox() {
  ...
  useEffect(()=>{
      ...
      axios.get(`https://auctions-api.stagingzix.xyz/mail/outbox/${authState.id}`, head).then((res)=>{
  ...
  const displayMessage = (name) =>{
      axios.get(`https://auctions-api.stagingzix.xyz/auth/fetchy/${name.senderId}`).then((respo)=>{
  ...
  const deleteMessage = (name) =>{
        ...
        axios.put(`https://auctions-api.stagingzix.xyz/mail/message/deleteoutbox/${name.id}`).then((res)=>{
        });
        let mails = [...mailList];
